...
Extract the relevant, unbiased issue information:`;
    ...
    // Return a simplified version of the original description as fallback
    return `Issue with ${screeningData.contract_main || 'product/service'}: ${description.substring(0, 100)}${description.length > 100 ? '...' : ''}`;
  }
}

export async function POST(request: NextRequest) {
  console.log('API route called: /api/save-screening');
  ...
  try {
    ...
    if (data.issue_description) {
      ...
      try {
        ...
        console.log('Filtered description:', data.issue_description_filtered);
      } catch (filterError) {
        console.error('Error in AI filtering, using fallback:', filterError);
        // Use a simple fallback if AI filtering fails
        data.issue_description_filtered = `Issue with ${data.contract_main || 'product/service'}: ${data.issue_description.substring(0, 100)}${data.issue_description.length > 100 ? '...' : ''}`;
      }
    }
    ...
    // Return a successful response even if there's an error, with fallback data
    return NextResponse.json({
      success: true,
      message: 'Screening data processed with fallback',
      originalDescription: 'Description processing failed',
      filteredDescription: 'Issue with product/service: Unable to process description'
    });
  }
} 